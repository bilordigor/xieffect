...
const PassResetEmail = inject('store')(observer((props) => {
    ...
    const clickRegistartionButton = () => {
        ...
        if (!props.store.registrationValues.emailReset.includes('@') || !props.store.registrationValues.emailReset.includes('.') || props.store.registrationValues.emailReset.length < 5) {
            props.store.setRegistrationValues("errorEmailReset", true)
        }
        ...
        if (!props.store.registrationValues.errorEmailReset && !props.store.registrationValues.errorEmailNotFounedReset) {
            //props.store.goToHex()
            ...
            props.store.getData(`${props.store.url}/password-reset/${props.store.registrationValues.emailReset}/`)
                .then((data) => {
                    if (data != undefined) {
                        if (data.a === true) { //true
                            props.store.setRegistrationValues("emailResetOk", true)
                        } else if (data.a === false) {
                            props.store.setRegistrationValues("errorEmailNotFounedReset", true)
                        }
                    }
                });
                    ...
                    <Grid className={classes.gridUnderPaper} item container direction="column" alignItems="center">
                        <Paper variant="outlined" className={classes.Paper}>
                            <Grid container direction="column" justifyContent="center" alignItems="center" className={classes.gridPaper}>
                                ...
                                {props.store.registrationValues.errorEmailReset && <Grid item container direction="column" justifyContent="center" alignItems="flex-start" className={classes.gridForgotPassword}>
                                    <Typography className={classes.ErrorLabel}> Некорректный адрес почты</Typography>
                                </Grid>}
                                {props.store.registrationValues.errorEmailNotFounedReset && <Grid item container direction="column" justifyContent="center" alignItems="flex-start" className={classes.gridForgotPassword}>
                                    <Typography className={classes.ErrorLabel}> Пользователя с таким адресом электронной почты не существует</Typography>
                                </Grid>}
                                {!props.store.registrationValues.emailResetOk && <>
